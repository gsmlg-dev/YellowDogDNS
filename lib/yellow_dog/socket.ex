defmodule YellowDog.Socket do
  @type t :: YellowDog.Socket.Protocol.t()
  ...
  defmodule Error do
    ...
    def exception(reason: reason) do
      message =
        cond do
          msg = YellowDog.Socket.TCP.error(reason) ->
            msg

          msg = YellowDog.Socket.SSL.error(reason) ->
            msg
  ...
  ## Supported URIs
  * `tcp://host:port` for YellowDog.Socket.TCP
  * `ssl://host:port` for YellowDog.Socket.SSL
  * `ws://host:port/path` for YellowDog.Socket.Web (using YellowDog.Socket.TCP)
  * `wss://host:port/path` for YellowDog.Socket.Web (using YellowDog.Socket.SSL)
  * `udp://host:port` for YellowDog.Socket:UDP
  ## Example
      { :ok, client } = YellowDog.Socket.connect "tcp://google.com:80"
      client.send "GET / HTTP/1.1\r\n"
      client.recv
  """
  @spec connect(String.t() | URI.t()) :: {:ok, YellowDog.Socket.t()} | {:error, any}
  ...
  def connect(%URI{scheme: "tcp", host: host, port: port}) do
    YellowDog.Socket.TCP.connect(host, port)
  end

  def connect(%URI{scheme: "ssl", host: host, port: port}) do
    YellowDog.Socket.SSL.connect(host, port)
  end

  def connect(%URI{scheme: "ws", host: host, port: port, path: path}) do
    YellowDog.Socket.Web.connect(host, port || @default_port_ws, path: path)
  end

  def connect(%URI{scheme: "wss", host: host, port: port, path: path}) do
    YellowDog.Socket.Web.connect(host, port || @default_port_wss, path: path, secure: true)
  end
  ...
  occurs, see `connect`.
  """
  @spec connect!(String.t() | URI.t()) :: YellowDog.Socket.t() | no_return
  def connect!(uri) when uri |> is_list or uri |> is_binary do
    connect!(URI.parse(uri))
  end
  ...
  * `udp://host:port` for YellowDog.Socket:UDP
  ## Example
      { :ok, server } = YellowDog.Socket.listen "tcp://*:1337"
      client = server |> YellowDog.Socket.accept!(packet: :line)
      client |> YellowDog.Socket.Stream.send(client.recv)
      client |> YellowDog.Socket.Stream.close
  """
  @spec listen(String.t() | URI.t()) :: {:ok, YellowDog.Socket.t()} | {:error, any}
  ...
  def listen(%URI{scheme: "ws", host: host, port: port}) do
    YellowDog.Socket.Web.listen(port || @default_port_ws,
      local: [address: if(host == "*", do: "0.0.0.0", else: host)]
    )
  end

  def listen(%URI{scheme: "wss", host: host, port: port}) do
    YellowDog.Socket.Web.listen(port || @default_port_wss,
      secure: true,
      local: [address: if(host == "*", do: "0.0.0.0", else: host)]
    )
  end
  ...
  see `listen`.
  """
  @spec listen!(String.t() | URI.t()) :: YellowDog.Socket.t() | no_return
  def listen!(uri) when uri |> is_list or uri |> is_binary do
    listen!(URI.parse(uri))
  end
  ...
  defdelegate equal?(self, other), to: YellowDog.Socket.Protocol

  defdelegate accept(self), to: YellowDog.Socket.Protocol
  defbang(accept(self), to: YellowDog.Socket.Protocol)

  defdelegate accept(self, options), to: YellowDog.Socket.Protocol
  defbang(accept(self, options), to: YellowDog.Socket.Protocol)

  defdelegate options(self, opts), to: YellowDog.Socket.Protocol
  defbang(options(self, opts), to: YellowDog.Socket.Protocol)

  defdelegate packet(self, type), to: YellowDog.Socket.Protocol
  defbang(packet(self, type), to: YellowDog.Socket.Protocol)

  defdelegate process(self, pid), to: YellowDog.Socket.Protocol
  defbang(process(self, pid), to: YellowDog.Socket.Protocol)

  defdelegate active(self), to: YellowDog.Socket.Protocol
  defbang(active(self), to: YellowDog.Socket.Protocol)

  defdelegate active(self, mode), to: YellowDog.Socket.Protocol
  defbang(active(self, mode), to: YellowDog.Socket.Protocol)

  defdelegate passive(self), to: YellowDog.Socket.Protocol
  defbang(passive(self), to: YellowDog.Socket.Protocol)

  defdelegate local(self), to: YellowDog.Socket.Protocol
  defbang(local(self), to: YellowDog.Socket.Protocol)

  defdelegate remote(self), to: YellowDog.Socket.Protocol
  defbang(remote(self), to: YellowDog.Socket.Protocol)

  defdelegate close(self), to: YellowDog.Socket.Protocol
  defbang(close(self), to: YellowDog.Socket.Protocol)
end

defprotocol YellowDog.Socket.Protocol do
  @doc """
...
defimpl YellowDog.Socket.Protocol, for: Port do
  ...
  def accept(self, options \\ []) do
    case :inet_db.lookup_socket(self) do
      {:ok, mod} when mod in [:inet_tcp, :inet6_tcp] ->
        YellowDog.Socket.TCP.accept(self, options)
  ...
  def options(self, opts) do
    :inet.setopts(self, YellowDog.Socket.arguments(opts))
  end
...
defimpl YellowDog.Socket.Protocol, for: Tuple do
  require Record
  ...
  def accept(self, options \\ []) when self |> Record.is_record(:sslsocket) do
    YellowDog.Socket.SSL.accept(self, options)
  end

  def options(self, opts) when self |> Record.is_record(:sslsocket) do
    YellowDog.Socket.SSL.options(self, opts)
  end
