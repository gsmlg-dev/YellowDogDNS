defmodule YellowDog.Socket.Web do
  @moduledoc ~S"""
  This module implements RFC 6455 WebYellowDog.Sockets.

  ## Client example

      socket = YellowDog.Socket.Web.connect! "echo.websocket.org"
      socket |> YellowDog.Socket.Web.send! { :text, "test" }
      socket |> YellowDog.Socket.Web.recv! # => {:text, "test"}

  ## Server example

      server = YellowDog.Socket.Web.listen! 80
      client = server |> YellowDog.Socket.Web.accept!

      # here you can verify if you want to accept the request or not, call
      # `YellowDog.Socket.Web.close!` if you don't want to accept it, or else call
      # `YellowDog.Socket.Web.accept!`
      client |> YellowDog.Socket.Web.accept!

      # echo the first message
      client |> YellowDog.Socket.Web.send!(client |> YellowDog.Socket.Web.recv!)
  ...
  @type error :: YellowDog.Socket.TCP.error() | YellowDog.Socket.SSL.error()
  ...
  @spec headers(%{String.t() => String.t()}, YellowDog.Socket.t(), Keyword.t()) :: %{
          String.t() => String.t()
        }
  defp headers(acc, socket, options) do
    case socket |> YellowDog.Socket.Stream.recv!(options) do
  ...
  """
  @spec connect({YellowDog.Socket.Address.t(), :inet.port_number()}) :: {:ok, t} | {:error, error}
  ...
  """
  @spec connect(
          {YellowDog.Socket.Address.t(), :inet.port_number()} | YellowDog.Socket.Address.t(),
  ...
  websockets or not.
  """
  @spec connect(YellowDog.Socket.Address.t(), :inet.port_number(), Keyword.t()) ::
          {:ok, t} | {:error, error}
  def connect(address, port, options) do
    try do
      ...
      e in [YellowDog.Socket.Error] ->
        {:error, e.message}

      e in [YellowDog.Socket.TCP.Error, YellowDog.Socket.SSL.Error] ->
        {:error, e.code}
    end
  end
  ...
  occurs.
  """
  @spec connect!({YellowDog.Socket.Address.t(), :inet.port_number()}) :: t | no_return
  def connect!({address, port}) do
    connect!(address, port, [])
  end
  ...
  websockets or not.
  """
  @spec connect!(YellowDog.Socket.Address.t(), :inet.port_number(), Keyword.t()) :: t | no_return
  def connect!(address, port, options) do
    {local, global} = arguments(options)

    mod =
      if local[:secure] do
        YellowDog.Socket.SSL
      else
        YellowDog.Socket.TCP
      end
    ...
    client |> YellowDog.Socket.packet!(:raw)

    client
    |> YellowDog.Socket.Stream.send!([
      "GET #{path} HTTP/1.1",
      "\r\n",
      headers,
      "Host: #{address}:#{port}",
      ...
      "Connection: Upgrade",
      "\r\n",
      "Sec-WebYellowDog.Socket-Key: #{handshake}",
      "\r\n",
      if(protocols,
        do: ["Sec-WebYellowDog.Socket-Protocol: #{Enum.join(protocols, ", ")}", "\r\n"],
        else: []
      ),
      if(extensions,
        do: ["Sec-WebYellowDog.Socket-Extensions: #{Enum.join(extensions, ", ")}", "\r\n"],
        else: []
      ),
      "Sec-WebYellowDog.Socket-Version: 13",
      "\r\n",
      "\r\n"
    ])

    client |> YellowDog.Socket.packet(:http_bin)
    {:http_response, _, 101, _} = client |> YellowDog.Socket.Stream.recv!(global)
    headers = headers(%{}, client, local)

    if String.downcase(headers["upgrade"] || "") != "websocket" or
         String.downcase(headers["connection"] || "") != "upgrade" do
      client |> YellowDog.Socket.close()
  ...
  def accept!(%W{socket: socket, key: nil}, options) do
    ...
    client = socket |> YellowDog.Socket.accept!(global)
    client |> YellowDog.Socket.packet!(:http_bin)

    path =
      case client |> YellowDog.Socket.Stream.recv!(global) do
  ...
  def accept!(%W{socket: socket, key: key}, options) do
    ...
    socket |> YellowDog.Socket.packet!(:raw)
    ...
    |> YellowDog.Socket.Stream.send!([
      ...
      "Connection: Upgrade",
      "\r\n",
      "Sec-WebYellowDog.Socket-Accept: #{key(key)}",
      "\r\n",
      "Sec-WebYellowDog.Socket-Version: 13",
      "\r\n",
      if(extensions,
      ...
      if(protocol, do: ["Sec-WebYellowDog.Socket-Protocol: ", protocol, "\r\n"], else: []),
      "\r\n"
    ])
  ...
  def remote(%W{socket: socket}) do
    socket |> YellowDog.Socket.remote()
  end
  ...
  defp recv(%W{socket: socket, version: 13}, mask, length, options) do
    length =
      cond do
        length == 127 ->
          case socket |> YellowDog.Socket.Stream.recv(8, options) do
            {:ok, <<length::64>>} ->
              length
        ...
        length == 126 ->
          case socket |> YellowDog.Socket.Stream.recv(2, options) do
            {:ok, <<length::16>>} ->
              length
    ...
    case length do
      ...
      length ->
        if mask do
          case socket |> YellowDog.Socket.Stream.recv(4, options) do
            {:ok, <<key::32>>} ->
              if length > 0 do
                case socket |> YellowDog.Socket.Stream.recv(length, options) do
                  {:ok, data} ->
                    {:ok, unmask(key, data)}
  ...
  def send(%W{socket: socket, version: 13, mask: mask}, {opcode, data}, options)
    ...
    socket
    |> YellowDog.Socket.Stream.send(<<1::1, 0::3, opcode(opcode)::4, forge(mask, data)::binary>>)
  end

  def send(%W{socket: socket, version: 13, mask: mask}, {:fragmented, :end, data}, options) do
    mask = if Keyword.has_key?(options, :mask), do: options[:mask], else: mask

    socket |> YellowDog.Socket.Stream.send(<<1::1, 0::3, 0::4, forge(mask, data)::binary>>)
  end
  ...
  def send(
    ...
    socket |> YellowDog.Socket.Stream.send(<<0::1, 0::3, 0::4, forge(mask, data)::binary>>)
  end
  ...
  def send(%W{socket: socket, version: 13, mask: mask}, {:fragmented, opcode, data}, options) do
    ...
    socket
    |> YellowDog.Socket.Stream.send(<<0::1, 0::3, opcode(opcode)::4, forge(mask, data)::binary>>)
  end
