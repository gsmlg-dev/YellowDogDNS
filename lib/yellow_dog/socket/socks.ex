defmodule YellowDog.Socket.SOCKS do
  ...
  defp pre(address, port, options) do
    ...
    YellowDog.Socket.TCP.connect(address, port, options)
  end
  ...
  defp handshake(socket, 4, auth, {address, port}) do
    ...
    case YellowDog.Socket.Address.parse(address) do
      {a, b, c, d} ->
        case socket
  ...
  defp response(socket, 4) do
    case socket |> YellowDog.Socket.Stream.recv(8) do
      # request granted
  ...
  defp post(socket, options) do
    cond do
      options[:mode] == :active ->
        socket |> YellowDog.Socket.active()

      options[:mode] == :once ->
        socket |> YellowDog.Socket.active(:once)
