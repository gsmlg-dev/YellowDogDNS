defmodule YellowDog.Socket.Host do
  defstruct [:name, :aliases, :type, :length, :list]

  @type t :: %YellowDog.Socket.Host{
          name: String.t(),
          aliases: [String.t()],
          type: :inet | :inet6,
          length: integer,
  ...
  """
  @spec by_address(YellowDog.Socket.Address.t()) :: {:ok, t} | {:error, :inet.posix()}
  def by_address(address) do
    case :inet.gethostbyaddr(YellowDog.Socket.Address.parse(address)) do
      {:ok, host} ->
        {:ok, convert(host)}
  ...
  """
  @spec by_address!(YellowDog.Socket.Address.t()) :: t | no_return
  def by_address!(address) do
    case :inet.gethostbyaddr(YellowDog.Socket.Address.parse(address)) do
      {:ok, host} ->
        convert(host)
