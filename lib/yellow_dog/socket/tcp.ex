defmodule YellowDog.Socket.TCP do
  @moduledoc """
  ...
  * `:size` sets the max length of the packet body, see `inet:setopts`
  ...
  ## Examples
      ...
      server = YellowDog.Socket.TCP.listen!(1337, packet: :line)

      client = server |> YellowDog.Socket.accept!
      client |> YellowDog.Socket.Stream.send!(client |> YellowDog.Socket.Stream.recv!)
  ...
  """
  @spec connect({YellowDog.Socket.Address.t(), :inet.port_number()}) ::
  ...
  """
  @spec listen :: {:ok, t} | {:error, YellowDog.Socket.Error.t()}
  def listen do
    listen(0, [])
  end
  ...
  """
  @spec options(t | YellowDog.Socket.SSL.t() | port, Keyword.t()) ::
          :ok | {:error, YellowDog.Socket.Error.t()}
  def options(socket, options) when socket |> Record.is_record(:sslsocket) do
    YellowDog.Socket.SSL.options(socket, options)
  end
  ...
  """
  @spec options!(t | YellowDog.Socket.SSL.t() | port, Keyword.t()) :: :ok | no_return
  ...
  def arguments(options) do
    ...
    YellowDog.Socket.arguments(global) ++
      Enum.flat_map(local, fn
        ...
        {:local, options} ->
          Enum.flat_map(options, fn
            {:address, address} ->
              [{:ip, YellowDog.Socket.Address.parse(address)}]
