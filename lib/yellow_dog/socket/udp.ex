defmodule YellowDog.Socket.UDP do
  @moduledoc """
  ...
  * `:broadcast` enables broadcast sending
  ...
  ## Examples
      ...
      server = YellowDog.Socket.UDP.open!(1337)

      { data, client } = server |> YellowDog.Socket.Datagram.recv!
      server |> YellowDog.Socket.Datagram.send! data, client
    ...
    YellowDog.Socket.arguments(global) ++
      Enum.flat_map(local, fn
        ...
        {:multicast, options} ->
          Enum.flat_map(options, fn
            {:address, address} ->
              [{:multicast_if, YellowDog.Socket.Address.parse(address)}]
