defmodule YellowDog.Mnesia.Table do
  ...
  @moduledoc """
  Defines a YellowDog.Mnesia Table schema for Mnesia


  ## Usage

  You can define an Mnesia Table by calling `use YellowDog.Mnesia.Table` with a few
  options in your module.

  ```
  defmodule Blog.Post do
    use YellowDog.Mnesia.Table, attributes: [:id, :title, :content]
  end
  ```
  ...
  ```
  YellowDog.Mnesia.Table.create(Blog.Post)
  ```
  ...
  - `autoincrement` - If the table is of the type `:ordered_set`, setting this
    ...
    when writing records (using `YellowDog.Mnesia.Query.write/2`). Will return an error
  ...
  @typedoc "A YellowDog.Mnesia.Table module"
  @type name :: module()

  @typedoc "A YellowDog.Mnesia.Table record data struct"
  @type record :: struct()
  ...
  Every defined `YellowDog.Mnesia.Table` via the `use` macro, will export this
  ...
  @doc """
  Creates a YellowDog.Mnesia Table for Mnesia.
  ...
  passed module isn't a YellowDog.Mnesia Table.
  ...
  def create(table, opts \\ []) do
    ...
    # Validate Options
    type = info.type
    auto = Keyword.get(opts.yellow_dog_mnesia, :autoincrement, false)
    ...
    cond do
      ...
      true ->
        ...
        :create_table
        |> YellowDog.Mnesia.Mnesia.call([table, mnesia_opts])
  ...
  @doc """
  Deletes a YellowDog.Mnesia Table for Mnesia.
  ...
  def delete(table) do
    ...
    :delete_table
    |> YellowDog.Mnesia.Mnesia.call([table])
    |> YellowDog.Mnesia.Mnesia.handle_result()
  end
  ...
  YellowDog.Mnesia.Table.create_copy(Users, :some_node@host_x, :disc_copies)
  ```
  """
  @spec create_copy(name, node, storage_type) :: :ok | {:error, any}
  def create_copy(table, node, type) do
    :add_table_copy
    |> YellowDog.Mnesia.Mnesia.call_and_catch([table, node, type])
    |> YellowDog.Mnesia.Mnesia.handle_result()
  end
  ...
  be removed (Mnesia/YellowDog.Mnesia must be stopped first).
  ...
  def delete_copy(table, node) do
    :del_table_copy
    |> YellowDog.Mnesia.Mnesia.call_and_catch([table, node])
    |> YellowDog.Mnesia.Mnesia.handle_result()
  end
  ...
  def move_copy(table, node_from, node_to) do
    :move_table_copy
    |> YellowDog.Mnesia.Mnesia.call_and_catch([table, node_from, node_to])
    |> YellowDog.Mnesia.Mnesia.handle_result()
  end
  ...
  use `YellowDog.Mnesia.Schema.set_storage_type/2` instead.
  ...
  ```
  YellowDog.Mnesia.Table.set_storage_type(MyTable, :node@host, :disc_copies)
  ```
  """
  ...
  def info(table, key \\ :all) do
    Definition.validate_table!(table)
    YellowDog.Mnesia.Mnesia.call(:table_info, [table, key])
  end

  @doc """
  Deletes all entries in the given YellowDog.Mnesia Table.
  ...
  This method can be accessed directly on the `YellowDog.Mnesia` module as well.
  ...
  YellowDog.Mnesia.Table.wait(Movies, :infinity)

  # Wait a maximum of 3 seconds until the two tables are ready
  YellowDog.Mnesia.wait([TableA, TableB])
  ```
  """
  ...
  def wait(tables, timeout \\ 3000) do
    ...
    YellowDog.Mnesia.Mnesia.call(:wait_for_tables, [tables, timeout])
  end
  ...
  defp handle_for_bang!(error) do
    YellowDog.Mnesia.Error.raise_from_code(error)
  end
end
