defmodule YellowDog.Mnesia.Query do
  require YellowDog.Mnesia.Mnesia
  require YellowDog.Mnesia.Error

  alias YellowDog.Mnesia.Query
  alias YellowDog.Mnesia.Mnesia
  alias YellowDog.Mnesia.Table
  alias YellowDog.Mnesia.Table.Definition

  @moduledoc """
  Module to read/write from YellowDog.Mnesia Tables.

  This module provides the most important transactional operations
  that can be executed on YellowDog.Mnesia Tables. Mnesia's "dirty" methods
  are left out on purpose. In almost all circumstances, these
  methods would be enough for interacting with YellowDog.Mnesia Tables, but
  ...
  within the context of a `YellowDog.Mnesia.Transaction`. Outside the
  ...
  ```
  # Will raise an error
  YellowDog.Mnesia.Query.read(Blog.Post, :some_id)
  ...
  # Write a record
  YellowDog.Mnesia.Query.write(%User{id: 3, name: "Some User"})

  # Delete a record by primary key
  YellowDog.Mnesia.Query.delete(User, id)

  # Delete a record by passing the full object
  YellowDog.Mnesia.Query.delete_record(%User{id: 4, name: "Another User"})
  ```


  ## Complex Queries

  YellowDog.Mnesia provides 3 ways of querying records based on some passed
  conditions:
  ...
  ```
  # Get all Movies
  YellowDog.Mnesia.Query.select(Movie, [])

  # Get all Movies named "Rush"
  YellowDog.Mnesia.Query.select(Movie, {:==, :title, "Rush"})
  ...
  YellowDog.Mnesia.Query.select(Movie, guards)
  ```
  """
  ...
  This converts them into simple YellowDog.Mnesia struct records of type
  `t:YellowDog.Mnesia.Table.record/0`. This is equivalent to calling
  `YellowDog.Mnesia.Query.Data.load/1` on the returned records. This option is
  ...
  terminated and executed again. YellowDog.Mnesia does this automatically
  ...
  @doc """
  Finds the YellowDog.Mnesia record for the given id in the specified table.
  ...
  ```
  YellowDog.Mnesia.Query.read(Blog.Post, 1)
  # => %Blog.Post{id: 1, ... }

  YellowDog.Mnesia.Query.read(Blog.Post, 2, lock: :write)
  # => %Blog.Post{id: 2, ... }

  YellowDog.Mnesia.Query.read(Blog.Post, :unknown_id)
  # => nil
  ```
  """
  ...
  @doc """
  Writes a YellowDog.Mnesia record to its Mnesia table.
  ...
  YellowDog.Mnesia record is `nil` and the table does not have autoincrement
  ...
  the table definition. (See `YellowDog.Mnesia.Table` for more details).


  ## Examples

  ```
  YellowDog.Mnesia.Query.write(%Blog.Post{title: "something", ... })
  # => %Blog.Post{id: 4, title: "something"}

  YellowDog.Mnesia.Query.write(%Blog.Author{username: "sye", ... })
  ...
  ```
  # Both are equivalent
  YellowDog.Mnesia.Query.all(Movie)
  YellowDog.Mnesia.Query.match(Movie, {:_, :_, :_, :_})
  ```
  """
  ...
  This method takes the name of a `YellowDog.Mnesia.Table` and a tuple pattern
  ...
  YellowDog.Mnesia.Query.match(Movie, {:_, "Rush", :_, :_}, lock: :write)

  # Get all movies directed by Tarantino
  YellowDog.Mnesia.Query.match(Movie, {:_, :_, :_, "Quentin Tarantino"})

  # Get all movies directed by Spielberg, in the year 1993
  YellowDog.Mnesia.Query.match(Movie, {:_, :_, 1993, "Steven Spielberg"})

  # Will raise exceptions
  YellowDog.Mnesia.Query.match(Movie, {:_, :_})
  YellowDog.Mnesia.Query.match(Movie, {:_, :_, :_})
  YellowDog.Mnesia.Query.match(Movie, {:_, :_, :_, :_, :_})
  ```
  """
  ...
  def match(table, pattern, opts \\ []) when is_tuple(pattern) do
    ...
    pattern = Tuple.insert_at(pattern, 0, table)
  ...
  def select(table, guards, opts \\ []) do
    ...
    guards = YellowDog.Mnesia.Query.Spec.build(guards, attr_map)
  ...
  YellowDog.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}])
  # => [%Movie{...}, ...]
  ```
  ...
  YellowDog.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: false)
  ...
  YellowDog.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: true)
  # => [%Movie{...}, ...]
  ```
  ...
  This method takes a `YellowDog.Mnesia.Table` name and a key, and deletes all
  ...
  YellowDog.Mnesia.Query.delete(Blog.Post, 10)
  ```
  """
  ...
  @doc """
  Delete the given YellowDog.Mnesia record object.

  This method accepts a `t:YellowDog.Mnesia.Table.record/0` object and deletes
  ...
  defp validate_match_pattern!(table, pattern) do
    ...
    unless same_size? do
      YellowDog.Mnesia.Error.raise("Match Pattern length is not equal to the no. of attributes")
    end
  end
  ...
  defp prepare_record_for_write!(table, record) do
    ...
    cond do
      ...
      is_nil(primary) ->
        YellowDog.Mnesia.Error.raise(
          "YellowDog.Mnesia records cannot have a nil primary key unless autoincrement is enabled"
        )
    end
  end
